...
<head>
    ...
    <style>
        ...
        /* Sejajarkan beberapa kolom ke kiri untuk keterbacaan */
        td:nth-child(2),
        td:nth-child(3),
        td:nth-child(4) {
            text-align: left;
...
<body onload="window.print()">
    ...
    <p class="periode">
        Periode: {{ $tanggal_awal ? \Carbon\Carbon::parse($tanggal_awal)->format('d-m-Y') : 'Awal' }} s/d
    ...
    <table>
        ...
        <tbody>
            @forelse($barangs as $i => $barang)
                <tr>
                    ...
                    <td>Rp {{ number_format($barang->total_harga) }}</td>
                    <td>{{ $barang->qty > 10 ? 'Aman' : ($barang->qty > 0 ? 'Terbatas' : 'Habis') }}</td>
                </tr>
            @empty
                <tr>
        ...
        <tfoot>
            <tr>
                ...
                <td colspan="2" style="font-weight: bold;">Rp {{ number_format($barangs->sum('total_harga')) }}</td>
            </tr>
        </tfoot>
    </table>
